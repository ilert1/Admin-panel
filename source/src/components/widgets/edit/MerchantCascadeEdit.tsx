...
import { CascadeSchema, MerchantCascadeSchema } from "@/api/enigma/blowFishEnigmaAPIService.schemas";
import { useAppToast } from "@/components/ui/toast/useAppToast";

const CASCADE_STATE = ["active", "inactive"];

export interface MerchantCascadeEditProps {
    id?: string;
    onOpenChange: (state: boolean) => void;
}

export const MerchantCascadeEdit = ({ id, onOpenChange }: MerchantCascadeEditProps) => {
    const controllerProps = useEditController<MerchantCascadeSchema>({
    ...
    const form = useForm<z.infer<typeof formSchema>>({
        ...
        defaultValues: {
            state: controllerProps?.record?.state || CASCADE_STATE[0]
        }
    });
